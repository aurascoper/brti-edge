#!/usr/bin/env bash
# =============================================================================
# Polymarket data-collector — drain-safe supervised launcher
# =============================================================================
# Same layered architecture as run-collector-supervised.sh (Kalshi side):
#
#   1. SLEEP PREVENTION  — caffeinate holds display/idle/disk/system assertions
#                          for the supervisor's lifetime.
#   2. CRASH RESTART     — relaunch on exit, with exponential backoff so a
#                          crash-loop can't hammer the CLOB API.
#   3. STALL WATCHDOG    — if no pm-book file has been written for
#                          STALE_SECONDS (default 300s; the collector flushes
#                          every 5s while subscribed), the socket is dead but
#                          the process is wedged — drain-restart it.
#
# DRAIN SAFETY: on every stop we send SIGTERM and grant GRACE_SECONDS (default
# 5s > the collector's 3s-per-rotator closeAsync timeout) for the gzip CRC32+
# size trailer to land, THEN SIGKILL.
#
# Run directly (simple, e.g. inside tmux):   bash scripts/run-polymarket-collector.sh
# Or under launchd (boot/sleep-wake survival): see com.polyterminal.polymarket-collector.plist
# =============================================================================
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
ROOT_DIR="$(cd "$APP_DIR/../.." && pwd)"
LOG_DIR="${PM_COLLECTOR_LOG_DIR:-$APP_DIR/logs/polymarket}"
STATE_DIR="$APP_DIR/ops/state"
STATUS="$STATE_DIR/polymarket-collector-supervisor.jsonl"
mkdir -p "$LOG_DIR" "$STATE_DIR"

# ---- tunables (all overridable via env) ----
GRACE_SECONDS="${PM_COLLECTOR_GRACE_SECONDS:-5}"     # SIGTERM -> wait -> SIGKILL
STALE_SECONDS="${PM_COLLECTOR_STALE_SECONDS:-300}"   # no pm-book write => stall
WATCH_INTERVAL="${PM_COLLECTOR_WATCH_INTERVAL:-30}"  # health-check cadence
MIN_UP_SECONDS="${PM_COLLECTOR_MIN_UP_SECONDS:-30}"  # shorter life => back off
MAX_BACKOFF="${PM_COLLECTOR_MAX_BACKOFF:-60}"

STOPPING=false
CHILD_PID=""
CAFFEINATE_PID=""
RESTARTS=0

now_iso() { date -u +%Y-%m-%dT%H:%M:%SZ; }

# Interruptible sleep: a FOREGROUND `sleep` defers trapped signals until it
# finishes (so SIGTERM could take WATCH_INTERVAL seconds to honor — longer than
# launchd's ExitTimeOut, which would SIGKILL us mid-drain and orphan the
# collector). Backgrounding + `wait` lets on_signal run within ~0s of SIGTERM.
nap() { sleep "$1" & wait "$!" 2>/dev/null; }

# Cap the launchd StandardErrorPath log (gzip-backpressure warns flood it).
# launchd holds the fd open, so truncate via cat-copy-back to preserve the
# inode — tail|mv would orphan the fd and the file would keep growing
# invisibly. Checked each health tick; fires >1MB.
ERR_LOG="$STATE_DIR/launchd-pm-collector.err.log"
cap_err_log() {
  [ -f "$ERR_LOG" ] || return 0
  [ "$(stat -f%z "$ERR_LOG" 2>/dev/null || stat -c%s "$ERR_LOG" 2>/dev/null || echo 0)" -gt 1048576 ] || return 0
  tail -n 5000 "$ERR_LOG" > "$ERR_LOG.captmp" && cat "$ERR_LOG.captmp" > "$ERR_LOG" && rm -f "$ERR_LOG.captmp"
}

log_event() {
  local ev="$1" reason="${2:-}" ts; ts="$(now_iso)"
  printf '{"ts":"%s","event":"%s","reason":"%s","child_pid":%s,"restarts":%s}\n' \
    "$ts" "$ev" "$reason" "${CHILD_PID:-null}" "$RESTARTS" >> "$STATUS"
  echo "[$ts] pm-supervisor: $ev${reason:+ — $reason}"
}

# Safe .env loader: simple KEY=VALUE lines only, no eval/command-substitution.
load_env() {
  local f="$1" line key val
  [ -f "$f" ] || return 0
  while IFS= read -r line || [ -n "$line" ]; do
    line="${line#export }"
    case "$line" in ''|\#*) continue ;; *=*) ;; *) continue ;; esac
    key="${line%%=*}"; val="${line#*=}"
    case "$key" in *[!A-Za-z0-9_]*) continue ;; esac
    val="${val%\"}"; val="${val#\"}"; val="${val%\'}"; val="${val#\'}"
    export "$key=$val"
  done < "$f"
}
load_env "$ROOT_DIR/.env"
load_env "$APP_DIR/.env"

# Shared-contract defaults: export so the collector sees them even under
# launchd's minimal environment. Anything already set (shell or .env) wins.
export PM_COLLECTOR_LOG_DIR="$LOG_DIR"
export PM_COLLECTOR_ASSETS="${PM_COLLECTOR_ASSETS:-btc,eth,sol}"
export PM_COLLECTOR_REFRESH_MS="${PM_COLLECTOR_REFRESH_MS:-30000}"
export PM_COLLECTOR_HEARTBEAT_MS="${PM_COLLECTOR_HEARTBEAT_MS:-60000}"

# No credentials needed: the Polymarket CLOB market-data websocket is public.

# Resolve the collector entrypoint as a DIRECT child so SIGTERM reaches it.
if [ -x "$APP_DIR/node_modules/.bin/tsx" ]; then
  RUN=("$APP_DIR/node_modules/.bin/tsx" "src/polymarket/index.ts")
else
  RUN=(pnpm exec tsx src/polymarket/index.ts)
fi

newest_book_mtime() {
  local newest=0 m f
  for f in "$LOG_DIR"/pm-book-*.jsonl.gz; do
    [ -e "$f" ] || continue
    # BSD stat (macOS) first, GNU stat (Linux) fallback — without the
    # fallback the watchdog silently never fires on Linux.
    m="$(stat -f %m "$f" 2>/dev/null || stat -c %Y "$f" 2>/dev/null)" || continue
    [ "$m" -gt "$newest" ] && newest="$m"
  done
  echo "$newest"
}

stop_child() {
  local pid="$1" i=0
  [ -n "$pid" ] || return 0
  kill -0 "$pid" 2>/dev/null || return 0
  kill -TERM "$pid" 2>/dev/null
  while kill -0 "$pid" 2>/dev/null; do
    i=$((i + 1))
    if [ "$i" -ge "$GRACE_SECONDS" ]; then
      log_event sigkill "child $pid did not drain within ${GRACE_SECONDS}s"
      kill -KILL "$pid" 2>/dev/null
      break
    fi
    sleep 1
  done
}

on_signal() {
  $STOPPING && return
  STOPPING=true
  log_event supervisor_stop "signal received — draining child"
  stop_child "$CHILD_PID"
  [ -n "$CAFFEINATE_PID" ] && kill "$CAFFEINATE_PID" 2>/dev/null
  exit 0
}
trap on_signal TERM INT

# Hold sleep assertions for the supervisor's lifetime (-w waits on our PID).
if command -v caffeinate >/dev/null 2>&1; then
  caffeinate -dimsw "$$" &
  CAFFEINATE_PID="$!"
  log_event caffeinate "holding display/idle/disk/system assertions (pid $CAFFEINATE_PID)"
else
  log_event warn "caffeinate not found — system sleep can still cause coverage gaps"
fi

log_event supervisor_start "log_dir=$LOG_DIR grace=${GRACE_SECONDS}s stale=${STALE_SECONDS}s assets=$PM_COLLECTOR_ASSETS"

backoff=1
while true; do
  start_ts="$(date +%s)"
  ( cd "$APP_DIR" && exec "${RUN[@]}" ) &
  CHILD_PID="$!"
  log_event start "collector pid $CHILD_PID"

  # inner health-watch loop
  while kill -0 "$CHILD_PID" 2>/dev/null; do
    nap "$WATCH_INTERVAL"
    $STOPPING && break
    cap_err_log
    mt="$(newest_book_mtime)"
    if [ "$mt" -gt 0 ]; then
      age=$(( $(date +%s) - mt ))
      if [ "$age" -gt "$STALE_SECONDS" ]; then
        log_event stale_restart "no pm-book write in ${age}s (>${STALE_SECONDS}s)"
        stop_child "$CHILD_PID"
        break
      fi
    fi
  done

  wait "$CHILD_PID" 2>/dev/null; rc=$?
  up=$(( $(date +%s) - start_ts ))
  $STOPPING && break
  RESTARTS=$(( RESTARTS + 1 ))
  log_event child_exit "rc=$rc uptime_s=$up"

  if [ "$up" -lt "$MIN_UP_SECONDS" ]; then
    backoff=$(( backoff * 2 ))
    [ "$backoff" -gt "$MAX_BACKOFF" ] && backoff="$MAX_BACKOFF"
  else
    backoff=1
  fi
  log_event backoff "restarting in ${backoff}s"
  nap "$backoff"
done

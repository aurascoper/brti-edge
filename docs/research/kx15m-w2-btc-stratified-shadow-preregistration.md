...
| Listing fetched 3 h beyond each window edge (for W2″) | **correction** | A window that starts or ends inside a Thursday pause had no close beyond the edge to anchor the pause, so a perfect host failed G4 with a 2.00 h or 1.00 h gap. The padded listing anchors the pause; reconciliation and the scorer keep the exact span |

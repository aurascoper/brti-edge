...
| Residual instrument defects recorded, not patched (for W2″, 2026-09-22) | **disclosure — no code change** | Three defects predate this lock and remain. (1) The scorer and the close wrapper read every settled close time and every shadow timestamp before their window filter, so one unreadable row from an earlier window ends the run, which the wrapper reports as NO-GO (integrity). No such row existed at the lock. (2) A local settlement whose close time lies outside the window is dropped before the identity comparison, so a mismatch across the boundary reads as a missing capture under the 2% rule, not as a conflict. (3) A settlement whose ticker does not start with its own series is skipped by reconciliation yet still scored, so it can enter S1's n. A patch for (1) and (3) was written and reverted the same day: its series guard dropped valid fills, and its ticker check called every ETH and SOL settlement invalid, which would have voided every close. Each fix belongs to its own round with its own review |

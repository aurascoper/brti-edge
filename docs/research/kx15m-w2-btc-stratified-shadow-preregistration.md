...
| Settlement identity reconciled (for W2″, review of `700daae`) | **harshening** | Reconciliation compared ticker names only. It now fails the window as NO-GO (data) on a local settlement absent from the exchange listing, on a ticker whose close time or result differs between the two, on a conflicting duplicate, or on an empty listing. A malformed row or ticker counts only when its close time places it inside the window. The local file spans every window ever scored, so an earlier window's bad row must not fail this one, and a row that cannot be placed in time is not this window's to fail on. At the lock the file had 955 settled rows since Aug 6 and no malformed row |
| Scorer skips a row it cannot place in time (for W2″, review of `2d9d5b9`) | **neutral — hygiene** | The scorer read every settled row's close time, and every shadow row's timestamp, before its own window filter. One unreadable row from an earlier window ended the run, and the wrapper turns a failed run into NO-GO (integrity). Such a row is now skipped, and the close wrapper still reports one inside the span. Check: `test_w2_replay_scorer.py` |
| Scorer scores a ticker only under its own series (for W2″, review of `2d9d5b9`) | **harshening** | The stratum comes from the row's series (§3), and §8's unit of n is settled KXBTC15M tickers. A settlement whose ticker did not start with that series was skipped by reconciliation yet still scored, so it could enter S1's n. It is no longer scored |

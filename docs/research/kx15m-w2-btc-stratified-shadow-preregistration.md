...
| Settlement identity reconciled (for W2″, review of `700daae`) | **harshening** | Reconciliation compared ticker names only. It now fails the window as NO-GO (data) on a local settlement absent from the exchange listing, on a conflicting duplicate, on an empty listing, or on a ticker whose close time or result differs between the two while both lie inside the window. A malformed row counts only when its close time places it inside the window: the local file spans every window ever scored, and a row that cannot be placed in time is not this window's to fail on. At the lock that file had 6,657 rows, 957 of them settled KXBTC15M, and none malformed |
| Residual instrument defects recorded, not patched (for W2″, 2026-09-22) | **disclosure — no code change** | Three defects predate this lock and remain in both the baseline and this text. (1) The scorer and the close wrapper read every settled close time and every shadow timestamp before their window filter. One unreadable row from an earlier window ends the run: a scorer failure prints NO-GO (integrity), while an unreadable shadow timestamp raises the wrapper's own error, which prints no verdict and exits non-zero. No such row existed at the lock. (2) A local settlement whose close times all lie outside the window is dropped before the identity comparison, so a mismatch across the boundary reads as a missing capture under the 2% rule. A conflicting duplicate with one close inside the window still fails. (3) A settlement whose ticker does not start with its own series is skipped by reconciliation yet still scored, so it can enter S1's n; the reviewer reproduced 199 fills becoming 200 in both states. A patch for (1) and (3) was written and reverted the same day: its series guard dropped valid fills, and its ticker check named every ETH and SOL settlement invalid, which would have voided every close. Each fix belongs to its own round with its own review A fourth case is recorded and left alone: the wrapper now reads the gate line only at the start of a line, so a settled ticker whose name contains `gates: G7(FAIL)` no longer poses as one. The baseline refused such output by searching everywhere, so that input alone turns its NO-GO into a GO here. All 1,327 tickers in the archived listing use capitals, digits and hyphens, and no rule bars a stricter ticker test from failing a healthy window if the exchange ever issues another character |

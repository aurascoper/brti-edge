...
## 13. Arbitrage taxonomy across the estate

Written down 2026-08-16 so "the lane is open" can never again blur permission with entitlement. The estate runs several strategy families; only one is an open arbitrage lane, and each is fenced by its own gate. Nothing here inherits capital or language across the fences.

Directional (screener/transitions and hl_engine's OU-z-gated-by-OBI) is momentum and mean-reversion. It is not arbitrage; it carries directional risk, it lives on its own book (HL Book A), and it never borrows the word or the capital of the arb lanes. Carry arbitrage is the hl-mexc-basis line, now **HL↔Kraken** after MEXC's futures order path proved retail-API-blocked. It is the one open arbitrage lane, and it is gated: the funding-differential must clear the house's existing `funding_gap_regression.py` R²>0.02 rule and then a preregistration before any capital moves, funding-interval normalization is mandatory (both venues verified 1h fractional; a future non-1h venue must be caught by `funding_interval_h`, never assumed), and capital only ever splits two-venue at dust-canary scale. Spread capture is the Polymarket up/down maker (this document's subject), gated by its own Phase 2/3 battery; it is market-making, not arbitrage.

Foreclosed, and foreclosed for cause: single-binary "complete set" entries on Polymarket (Up+Down<$1) are a feed artifact of one unified book (§4.1); NegRisk converter racing is competed to ~$0.08 medians by incumbents with better latency (§4.2); MEXC futures execution is retail-API-blocked — `order/submit` returns code 401 "Not logged in" for API keys while the web session token works, and session-token bypass libraries were declined (ToS violation risks account+balance seizure, plaintext-token-in-script is the exact anti-pattern a prior incident forced us to clean up, and the scraper is brittle against the gate discipline the whole system rests on); Robinhood algorithmic trading is a closed book (the obi-execution-engine sandbox's own README records the net-negative verdict and there is no official trading API); and box spreads on American-style options carry early-assignment risk that breaks the "riskless" premise. One transport note worth keeping: MEXC's Akamai front end 403s python-default User-Agents on the order endpoint, fixed with a browser UA — that is a WAF-compat detail, not a permission bypass, and reads/leverage were never gated. The corollary policy: the basis tape's 6-symbol universe stays frozen for continuity; any screener-informed extension is a dated, additive change made at a Phase-1 analysis boundary, never a silent mid-window edit.

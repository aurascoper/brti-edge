...
- **Holdout eligibility policy.** A collector window is eligible to serve as an official validation holdout for frozen policies (e.g. `BTC_YES_LATE_ASIA_v1`) iff: (1) it spans ≥ **30** UTC hour buckets, (2) **every** channel has ≥ **99%** coverage, and (3) **no** channel has a continuous-missing-hour gap > **1 hour**. Worst-channel aggregation is intentional — replay needs every stream intact, not an average. These three constants are **policy, not tunables**: changing them retroactively breaks pass/fail interpretation of previously-scored holdouts. Enforced by `isContinuousHoldoutEligible()` in `apps/data-collector/src/adequacyReport.ts`; surfaced in the adequacy report as `continuous_holdout_eligible: true|false`. Independent of the structural-adequacy verdict — a window can be `Adequate for replay harness work? YES` while `continuous_holdout_eligible: false` (this is exactly the prerun shape before 2026-06-02).
